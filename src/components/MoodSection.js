...
import { Wind, Feather, Sparkles, Heart, Calendar, RotateCcw, ShieldCheck, ArrowRight } from "lucide-react";
...
export default function MoodSection({ showCardContainer = true }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved) {
        ...
        if (todayEntry) {
          const match = MOODS.find((m) => m.id === todayEntry.moodId);
          if (match) setSelectedMood(match);
        }
      }
    } catch (e) {
    ...
    <div className="w-full flex flex-col items-center">
      ...
      <div className="text-center space-y-2 mb-6">
        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs bg-[#FDF7E5] text-[#635322] font-medium border border-[#F7E6B5]">
          <Heart size={13} className="text-[#E08736] fill-[#E08736]/20" />
          <span>เช็คอารมณ์ประจำวัน</span>
        </span>
        <h2 className="text-2xl sm:text-3xl font-semibold text-[#3A3A3A] tracking-tight">
          วันนี้รู้สึกยังไงบ้าง?
        </h2>
        <p className="text-sm text-[#7A7A7A] max-w-sm mx-auto">
          ซื่อสัตย์กับตัวเองได้เสมอ ไม่มีอารมณ์ไหนที่ผิดเลยนะ
        </p>
      </div>
        ...
        {MOODS.map((m) => {
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center justify-center p-2.5 sm:p-4 rounded-2xl border transition-all cursor-pointer active:scale-95 ${
                isSelected
                  ? `${m.color} ${m.borderColor} shadow-md scale-105 ring-2 ring-offset-2 ring-[#B8DCC8]`
                  : "bg-white/85 border-[#EFEAE1] hover:bg-[#FFFDF8] hover:border-[#DCD5C5] shadow-xs hover:scale-102"
              }`}
            >
              <span className="text-2xl sm:text-4xl mb-1 sm:mb-1.5 transition-transform hover:scale-110">
                {m.emoji}
              </span>
              <span className={`text-[11px] sm:text-xs font-medium whitespace-nowrap ${isSelected ? m.textColor : "text-[#666]"}`}>
                {m.label}
              </span>
            </button>
          );
      ...
      <AnimatePresence mode="wait">
        {selectedMood && (
          <motion.div
            ...
            className={`w-full max-w-md rounded-2xl p-5 sm:p-6 border shadow-sm space-y-4 mb-6 ${selectedMood.color} ${selectedMood.borderColor}`}
          >
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="text-xs text-[#7A7A7A] hover:text-[#333] px-2 py-1 rounded-md hover:bg-black/5 transition-colors"
                title="ซ่อนคำแนะนำ"
              >
                ย่อเก็บ
              </button>
                ...
                {selectedMood.actions.map((act, idx) => {
                  ...
                  return (
                    <Link
                      key={idx}
                      href={act.href}
                      className={`flex items-center justify-between px-3.5 py-2.5 rounded-xl text-xs sm:text-sm font-medium transition-all shadow-xs hover:opacity-90 active:scale-[0.98] ${act.color}`}
                    >
      ...
      {/* Local Mood History */}
      {mounted && moodHistory.length > 0 && (
        <div className="w-full max-w-md pt-4 border-t border-[#EFEAE1]/70 mb-4">
          <div className="flex items-center justify-between text-xs text-[#7A7A7A] mb-2.5">
            ...
            <button
              ...
              className="text-[11px] text-[#A0A0A0] hover:text-[#555] flex items-center gap-1 cursor-pointer transition-colors"
          ...
          <div className="flex items-center justify-start gap-2 overflow-x-auto py-1 scrollbar-none">
            {moodHistory.map((item, idx) => (
              <div
                key={idx}
                className="flex flex-col items-center bg-white/80 border border-[#EFEAE1] rounded-xl px-2.5 py-1.5 text-center min-w-[50px] shadow-xs shrink-0"
              >
  ...
  return (
    <section className="w-full max-w-2xl mx-auto rounded-3xl bg-white/70 backdrop-blur-sm border border-[#EFEAE1] p-6 sm:p-10 shadow-sm relative overflow-hidden">
      {/* Decorative ambient gradient backdrop */}
      <div className="absolute -top-16 -right-16 w-44 h-44 rounded-full bg-[#FDF7E5]/60 filter blur-2xl pointer-events-none" />
      <div className="absolute -bottom-16 -left-16 w-44 h-44 rounded-full bg-[#E2F2E9]/60 filter blur-2xl pointer-events-none" />
